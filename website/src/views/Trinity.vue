...
export default {
  ...
  data() {
    return {
      ...
      mapUrls: [
        ...
        // TODO: Update for Trinity HAYDEN HERE
        //344, 2, 9
      ],
      currentIndex: 0,
      nextIndex: 1,
  ...
  created() {
    ...
    this.rotateMaps();
    // Fetch arrival times
    fetch("/gtfsmap-realtime/feed/ccta-vt-us/arrivals?stopCode=805758")
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          this.arrivals.push({
            stop_title: "Trinity Campus (Colchester Ave) Towards Winooski",
            data: data.data,
          });
        }
      })
      .catch((error) => console.error("Error fetching arrival times:", error));
    fetch("/gtfsmap-realtime/feed/ccta-vt-us/arrivals?stopCode=805823")
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          this.arrivals.push({
            stop_title:
              "Trinity Campus(Colchester Ave) Towards Downtown / UVM Campus",
            data: data.data,
          });
        }
      })
      .catch((error) => console.error("Error fetching arrival times:", error));
    fetch("/gtfsmap-realtime/feed/ccta-vt-us/arrivals?stopCode=805620")
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          this.arrivals.push({
            stop_title: "Riverside Ave at Salmon Run (Towards Winooski)",
            data: data.data,
          });
        }
      })
      .catch((error) => console.error("Error fetching arrival times:", error));
    fetch("/gtfsmap-realtime/feed/ccta-vt-us/arrivals?stopCode=806000")
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          this.arrivals.push({
            stop_title: "Riverside Ave Towards Downtown",
            data: data.data,
          });
        }
      })
